...
On-device LLM extractor for unmatched SMS (measure: unparsed rate should drop toward ~0); optional in-app model download flow (model files are not bundled in the APK); monthly narrative insight from aggregate JSON via the same local model; **in-app assistant (ADR 0006, amended 2026-07-10)** — natural-language Q&A over local transactions/recurring/insights via a constrained intent whitelist + deterministic QueryEngine, answers grounded in SQL results only, no advice framing, inference fully on-device.
**Exit criteria:** Unparsed rate < 2% on a fresh bank you never wrote templates for; extraction works fully offline once the model is downloaded (asserted with airplane-mode QA); narrative renders from aggregates with zero raw text in the prompt (asserted in tests); every number in an assistant answer is traceable to a QueryEngine result and out-of-whitelist intents refuse gracefully (asserted in tests).
