...
- Findings: T-075's task item in TASKS.md carried an AC line but no `Evidence:` line, and I could find no WORKLOG.md entry for the session that produced commit d1d68b6 (`[T-075] Add on-device LLM runtime foundation`) — only the two earlier BLOCKED entries and the ADR 0008 pin entry. Per COLLABORATION §4 item 1 ("AC met, proven by tests — don't trust summary") and item 7 ("WORKLOG entry complete"), I have no evidence trail to verify against, so I cannot record PASS regardless of code quality.
- Independent verification I could still do: read every touched file directly (`lib/intelligence/llm/llm_runtime.dart`, `android/.../intelligence/LlmBridge.kt`, the `MainActivity.kt` diff, `lib/features/settings/settings_screen.dart`, both new test files, `docs/architecture.md`/`docs/privacy.md` diffs, ADR 0008). Findings: pinned URL/size/SHA-256 in `LlmBridge` match ADR 0008 exactly; `AppConstants.enableLocalLlm` defaults `false`; device-support gate checks both `MIN_TOTAL_MEMORY_BYTES` and `isLowRamDevice`; `downloadModel()` only promotes the partial file after both size and SHA-256 verify, and resumes via HTTP Range; `extractJson()` does strict-schema validation with exactly one retry; the `MainActivity` diff is additive (new `com.paisatrack/llm` channel + a shared executor/error-code helper) and doesn't change `PlatformEmbedder`'s behavior — it catches `PlatformException` without branching on `error.code`, so the renamed failure code (`embedder_failure` → `inference_failure`) is cosmetic, not a regression. Both new test files are non-vacuous (typed-reason assertions, single-method-call assertion, schema retry/reject cases, UI download/delete wiring with a fake runtime). GitNexus `impact(PlatformLlmRuntime, upstream, summaryOnly)`: LOW, 6 impacted symbols, 1 process, confined to the new Llm module. `detect_changes(compare, ab0fdf0)` reported `risk_level: critical`, but that's driven by sheer breadth (119 changed symbols across 16 files, mostly brand-new code) rather than a dangerous edit to a shared/frozen symbol — no change to the frozen §6.2 record contract or existing SMS/embedder ingestion paths.
- Could not run `flutter analyze`/`flutter test` myself: this dispatched session has no shell-command approval available (same documented limitation as the 2026-07-11 T-070/T-018/T-060 review sessions), so I could not substitute a real test run for the missing evidence.
- Board hygiene: T-075 → `Review: CHANGES`, moved from In Review back to In Progress (@codex), with the specific ask spelled out on the task: run `flutter analyze --no-pub` + `flutter test --no-pub` (focused files + full suite) on the device toolchain, append the missing WORKLOG entry, add an `Evidence:` line to the task. T-076's `Depends` line updated to reflect T-075's new state.
- Open questions: none.
...
- Did: (spec) wrote docs/assistant-nlq.md (T-083) — the intent JSON schema, the six-intent MVP whitelist, the QueryEngine→repository mapping, the grounding contract (numbers only from QueryEngine; renderer test as the T-076 exit criterion), refusal/no-advice/on-device rules, and a T-076 test plan. Same spec-before-build pattern as T-066. (code) hardened `SmsFixtureDonation.sanitize`: T-080 untitled-leading + payee/counterparty name masking and end-of-string greeting names (Title-Case `_greetingName`/`_leadingName`/`_payeeName`; payee rule requires a >=2-word name so single-token merchants survive); T-081 UPI-VPA/email handle masking (local-part → `<VPA>`, PSP/domain retained, runs before `_longDigits`); T-082 currency negative-lookbehind on `_longDigits` so comma-less currency-adjacent amounts (e.g. "Rs 250000") are no longer destroyed.
- Files: docs/assistant-nlq.md (new), lib/features/dev/sms_fixture_donation.dart, test/features/dev/sms_fixture_donation_test.dart (+6 tests), docs/privacy.md (coverage wording now describes VPA + untitled/payee masking and the residual Title-Case best-effort caveat), TASKS.md, WORKLOG.md.
- Evidence: NOT run through `flutter test`/`flutter analyze` — this Linux sandbox has no Dart/Flutter (documented wrong-arch limitation), so the four tasks sit In Review for @codex to run on the device toolchain/CI. Regex behavior pre-verified with an equivalent ECMAScript harness (Dart RegExp shares JS semantics incl. variable-length lookbehind): 8/8 cases PASS, covering all new AC scenarios AND the two pre-existing tests (no regression). GitNexus MCP not connected in-session and the CLI is wrong-arch; manual blast-radius trace instead — `sanitize`←`fixture()`←UnparsedSmsScreen donation path only: LOW, dev-screen-only, no production ingestion path touched.
- Decisions: (1) token `<VPA>` chosen over reusing `<ACCOUNT>` for handles — clearer provenance for template authors. (2) Name masking is intentionally conservative-by-casing: it can slightly over-mask a Title-Case two-word merchant after "to/from" (e.g. "to State Bank") and can miss an oddly-cased name — acceptable because the human approves the exact untruncated preview before any copy (defense in depth), and refinement can follow once real Kotak/Central fixtures (T-067) exist. (3) Amount rule keys on currency-adjacency per the AC, not a full amount parser.
- Open questions: whether @codex/@human want a stricter merchant vs. person disambiguation once real fixtures land — deferred.
- Board/automation note: T-080/T-081/T-082 + T-083 are In Review but marked `@claude build / @codex review` (inverse of the usual codex-builds/claude-reviews flow). The §7 handoff dispatches @claude on any In Review item, so an armed loop could misroute review back to @claude — @human should route the @codex review (or run `handoff.sh` deliberately). Not committed by me; left staged for @human.
- Next: on PASS, T-083 unblocks T-076; the sanitizer trio closes the T-071 privacy findings.

## 2026-07-12 @claude — Grooming: Phase 4 assistant + donation-sanitizer follow-ups (@human-directed)

- Did: board grooming only, no code. (1) Fixed an ID collision: the three T-071 donation-sanitizer follow-ups had been logged as T-077/T-078/T-079, but T-077 (Save exports) and T-078 (Weekly review) are already Done — renumbered the follow-ups to T-080/T-081/T-082 (never-reuse rule, COLLABORATION §2). T-079 retired/unused, not reassigned. (2) Groomed the trio out of the ungroomed backlog into Ready; @human assigned the build to @claude with independent review by @codex (`@claude build / @codex review`). (3) Split the Phase 4 assistant per §81 grooming: new spec task T-083 (@claude) — docs/assistant-nlq.md intent schema/whitelist/QueryEngine mapping/grounding contract, same pattern as the T-066 parser spec — placed in Ready since it needs only ADR 0006 (approved) and is parallel-safe with Phase 3. T-076 build set to @codex, Model `gpt-5.6-sol medium` (was gpt-5.6-terra high), Depends now T-083 + T-075 + T-064; still gated (T-075 In Review, T-064 not reached).
- Files: TASKS.md (header, Ready section, Phase 4 T-076, Backlog), docs/privacy.md (T-077/T-078 → T-080/T-081 cross-ref).
- Evidence: n/a (no code/tests). GitNexus not run — markdown board/doc edits only, no code symbols touched. Not committed — left for @human review/commit; note the handoff loop (§7) may act on the In Review T-075 independently.
- Decisions: spec stays @claude (specs are the architect's role, §1); build alone goes to @codex as @human directed. Assistant build remains gated — not moved to Ready, so it won't auto-dispatch codex.
- Open questions: whether @human wants the T-076 build split further (chat UI vs QueryEngine/intent layer) once T-083 lands — deferred to post-spec grooming.
- Next: on request, write the T-083 spec, or implement the T-080..T-082 sanitizer changes (needs the device Flutter toolchain for tests — sandbox is wrong-arch).

## 2026-07-11 @claude — Review T-060: PASS → Done

- Did: ran COLLABORATION.md §4 checklist on the sole `## In Review` item, T-060 (Insights screen), commit b3fd385.
- Findings: `InsightsScreen` watches `activeInsightsProvider` (non-dismissed rows for the current UTC period) and renders each kind through a closed `_presentation()` switch that only reads named numeric/string payload fields — verified the test that plants a `top_transaction_ids` value and asserts it never renders. Dismiss control writes `dismissed=true` via Drift and a widget test proves it persists to a real in-memory DB (not just a mock callback). Empty state and the new dashboard app-bar entry point (`IconButton` → `InsightsScreen`, `lib/features/dashboard/dashboard_screen.dart`) are both covered. Cross-checked the "9 focused widget tests" claim by counting `testWidgets` blocks directly: 3 new in `insights_screen_test.dart` + 6 in `dashboard_screen_test.dart` (1 new nav test, 5 pre-existing) = 9 total across the two touched files — claim holds when read as "total coverage," not "9 new tests." docs/architecture.md documents the field-allowlist rendering and dismissal semantics.
- Evidence: could not get shell approval to run `flutter test`/`flutter analyze` directly in this dispatched session (same limitation as the 2026-07-11 T-070/T-018 review), so verified by reading every touched lib/test/doc file directly instead of trusting the WORKLOG summary. GitNexus `detect_changes(scope: compare, base_ref: main~1)`: LOW risk, 0 affected processes. `impact(DashboardScreen, upstream, summaryOnly)`: LOW, 7 impacted symbols, 0 processes, 1 module (Dashboard) — matches the claimed nav-only blast radius, no frozen contract touched.
- Board hygiene: T-060 → Done.
- Process note (non-blocking): commit b3fd385 also bundles unrelated `.claude/pet/*` files (a dev-tooling easter egg, `.claude/commands/pet.md` + launcher) into the T-060 diff. No functional/privacy/security impact and not part of any frozen contract, so not blocking — but scope hygiene should keep unrelated tooling changes out of task commits going forward.
- Next: `## Ready` and `## In Review` are both empty; automated handoff has nothing to dispatch to @codex until a Ready item exists. No further action this session.
...
- T-070: `GenericParseRejection` enum (hardRejectTerm/noDirection/noAmount/noContextSignal) confirmed on `GenericTransactionParser`; `parse()` and `rejectionReason()` both delegate to one private `_evaluate()` returning `({record, rejection})` with exactly one non-null field — reported reason structurally cannot drift from the guard. Unparsed screen's `_UnparsedListView._genericReason` recomputes per row at display time (no schema change), specific label per stage, null case labelled "template-stage miss". Tests read directly and are non-vacuous: `generic_transaction_parser_test.dart` covers hard-reject precedence + each of the four guards + accepted(null) case with `parse()` agreement; `unparsed_sms_screen_test.dart` widget test asserts two different-body rows render distinct reason strings.
- T-018: `.github/workflows/ci.yml` guard step runs `build_runner` then `git status --porcelain -- '*.g.dart'`, failing on any modified OR untracked generated file (broadened from one hard-coded path); `flutter-action` pinned to `flutter-version: 3.44.4` for deterministic generator output; docs/development.md documents the guard + regeneration command.
- Evidence: sandbox could not get an approved shell to run `flutter analyze`/`flutter test` directly this session (command approval declined), so relied on (a) reading every touched lib/test/doc/ci file directly, (b) GitHub Actions: run 29146850910 (`gh run view`) on commit 42de9b1 — the exact commit containing both diffs — is green, meaning the T-018 guard step actually executed (build_runner + porcelain check) and the full `flutter test`/`flutter analyze` including the new T-070 tests passed for real, not just a YAML read, (c) GitNexus `impact(GenericTransactionParser, upstream)`: MEDIUM, 14 symbols, confined to `ParserCascade`/`_UnparsedListView`/tests — matches the claimed single-caller blast radius from the original session.
- Board hygiene: T-070, T-018 → Done. Done section was at 27 entries (over the 20-cap); archived the 7 oldest (T-040, T-047, T-037, T-038, T-039, T-036, T-035) verbatim to docs/tasks-archive.md, keeping Done at 20.
- Process note (non-blocking, informational only — already merged to main, not re-litigating): T-070/T-018 plus an entire untracked dashboard-UI-enhancement effort (UI_ENHANCEMENT_PLAN.md, `lib/features/dashboard/*`, format helpers, app_state_views) landed in one commit without individual `[T-xxx]` board entries for the dashboard work. Board coverage gap only — worth a task entry retroactively if that dashboard work needs its own review trail, but out of scope for this session (no `## In Review` item names it).
- Grooming: Phase 2.5b trust-loop gate (T-072..T-074) on the Phase 3 backlog is now fully Done, so I re-checked every Phase 3 task's `Depends` line against the Done list. Two had all dependencies cleared: T-058 (Burn-rate forecaster, depends only T-048 Done) and T-056 (Recurring screen, depends only T-055 Done) — moved both into `## Ready` verbatim (AC/Depends unchanged). Everything else in Phase 3 still depends on T-050 (Blocked — needs @human to pin an embedding artifact) or on a same-phase task not yet Done, so left in place.
- Files: TASKS.md, WORKLOG.md, docs/tasks-archive.md.
- Open questions: none.
- Next: `## Ready` now holds T-058 and T-056 (@codex, no further human input needed) — committing this session's work re-arms the automated handoff loop onto T-058. T-050/T-075/T-067/T-071 remain `## Blocked` pending @human decisions (embedding artifact, LLM model pin, fixture-gathering priority, donation-flow priority).
...
- Fix (lib/capture/sms_ingestion.dart): removed the settings watch; `SmsIngestor` gains optional `askDailyBudgetResolver` (`int Function()`), production passes `() => ref.read(appSettingsControllerProvider).valueOrNull?.askDailyBudget ?? AppConstants.askNowDailyBudget` — budget resolved lazily at decision time, subscription never rebuilds, slider changes apply on the next ingest (strictly better than the watch, which applied them via teardown). Back-compat: existing `askDailyBudget` int param preserved; all other constructor call sites unchanged.
- Regression test (test/capture/sms_ingestion_test.dart): 'settings emissions do not re-subscribe the capture stream' — single-subscription source, ingest one SMS, `setAskDailyBudget(1)` mid-stream, ingest another; asserts both raw_sms rows land (raw rows, not transactions, so T-025 echo suppression can't confuse the assertion). Any re-listen throws by construction.
- Evidence: impact preflight documented-unavailable (gitnexus npx SIGTERM in sandbox, same as prior sessions); manual caller analysis — SmsIngestor constructed only by smsCaptureBootstrapProvider, smsBackfillProvider (uses default budget, unchanged), and tests. Flutter cannot run in this sandbox (wrong arch): @human to re-run `flutter test` (expect 109/2/0) and commit the staged changes.
- Files: lib/capture/sms_ingestion.dart, test/capture/sms_ingestion_test.dart, TASKS.md, WORKLOG.md
- Next: green suite → T-046 → Done → @codex cleared for T-066/T-068.
...
## 2026-07-11 @claude — ADR 0007: pin T-050 embedding model; groom T-050 + T-071 to Ready

- Did: selected and documented the on-device embedding model (T-050 blocker): MediaPipe Universal Sentence Encoder float32 TFLite, generation-pinned GCS artifact; wrote ADR 0007 with source URL, GCS integrity metadata (size 6120274, MD5 5123e0bb50df2978272ca25bfc7194f1, CRC32C 7eb73e02), license/redistribution record (no redistribution grant → lazy download only, never bundled/committed), inference contract (Tasks Text 0.10.26 via platform channel, in-task tokenization, single float32 head), and rejected alternatives. Added tool/verify_embedder_model.py (stdlib download+size+MD5 verify → prints SHA-256; --dim prints real embedding dimension via mediapipe pip). Moved T-050 Blocked → Ready with the pin folded into its AC; moved T-071 Blocked → Ready on @human's recorded decision to prioritize (T-074 precondition Done).
- Files: docs/decisions/0007-on-device-embedding-model.md, tool/verify_embedder_model.py, TASKS.md, WORKLOG.md.
- Evidence: GCS object metadata read directly from the public JSON API for both `float32/1/` and `float32/latest/` objects (byte-identical, latest is mutable → code pins `1` + generation); pub.dev shows mediapipe_text 0.0.1 stale/master-channel-only → platform-channel decision; tasks-text 0.10.x confirmed on Maven Central.
- Open: SHA-256 and output dimension deliberately NOT fabricated — sandbox egress to storage.googleapis.com blocked. One run of `python3 tool/verify_embedder_model.py --dim` on a networked machine backfills both into ADR 0007; T-050's deterministic test must assert the dimension before T-051 stores embeddings.
- Next: @codex can pull T-050 (after the script run) and T-071; remaining blocked items are T-075 (needs the Phase 4 LLM artifact pinned — same treatment as ADR 0007) and T-067 (fixture gathering).
## 2026-07-11 @claude — ADR 0008: pin T-075 LLM artifact; T-075 → Ready

- Did: pinned the Phase 4 on-device LLM (T-075 blocker): Qwen2.5-1.5B-Instruct dynamic_int8 ekv4096 MediaPipe `.task` from the official litert-community HF org — ungated, Apache-2.0, revision-pinned URL, SHA-256 82968d0a6c3872cf016fdbcfc591571605f4c7fd2b0f64d2533df502cc6596b3 (authoritative HF LFS oid), size 1598556720, runtime `com.google.mediapipe:tasks-genai:0.10.24`. Documented why not Gemma (HF/Kaggle gating breaks the anonymous in-app download AC; ADR 0002 already lists Qwen2.5-1.5B as acceptable) and kept llama.cpp/GGUF as recorded fallback. Moved T-075 Blocked → Ready with the pin folded into its AC.
- Files: docs/decisions/0008-on-device-llm-model.md, TASKS.md, WORKLOG.md.
- Evidence: HF model API shows gated:false + license apache-2.0; HF tree API provides the LFS SHA-256 directly (no download needed, unlike ADR 0007); model README carries Google-published MediaPipe LLM Inference benchmarks (S25 Ultra int8/4096: ~163 tk/s prefill, ~26 tk/s decode, ~2.2 GB peak RSS); Maven confirms tasks-genai 0.10.24 latest (genai skipped 0.10.26 upstream, issue #6047).
- Decisions: ekv4096 over ekv1280 (same 1.6 GB, 3.2× context for T-076 prompts); PLAN §2 "Gemma-2B-class" amended by ADR to "1–2B-class open instruction-tuned model".
- Next: T-050 implementation (embedder service per ADR 0007).
## 2026-07-11 @claude — T-050 implementation (embedder per ADR 0007)

- Did: implemented the on-device embedder end to end. Dart `Embedder` abstraction + `PlatformEmbedder` over `com.paisatrack/embedder` (never throws: model-missing / PlatformException / MissingPluginException all → null so ingest never blocks) + `NoopEmbedder` + Riverpod provider. Kotlin `EmbedderBridge`: download of the generation-pinned artifact to a temp file, size+MD5 verification against the ADR 0007 pin, atomic install (no partial/unverified file ever left), Settings-facing delete, lazy MediaPipe `TextEmbedder` over a read-only mapped buffer, all on a dedicated single-thread executor. MainActivity gained only an additive channel registration + executor (same LOW pattern as T-077). Added `com.google.mediapipe:tasks-text:0.10.26` and the INTERNET permission with a scoped inline comment (model downloads only, per ADR 0002); privacy.md + architecture.md updated. Tests: 11 unit tests over a mocked channel (payload shape, trim, Float64→Float32 conversion, null/error/empty/malformed/no-plugin paths, lifecycle methods, Noop) + on-device `integration_test/embedder_determinism_test.dart` (download→verify→bit-exact repeat embeddings, distinct inputs distinct vectors, stable dimension printed for the ADR 0007 backfill).
- Files: lib/intelligence/models/embedder.dart, test/intelligence/models/embedder_test.dart, integration_test/embedder_determinism_test.dart, android/app/src/main/kotlin/com/paisatrack/intelligence/EmbedderBridge.kt, android/app/src/main/kotlin/com/paisatrack/MainActivity.kt, android/app/build.gradle.kts, android/app/src/main/AndroidManifest.xml, docs/architecture.md, docs/privacy.md, TASKS.md, WORKLOG.md.
- Evidence: NONE runnable in this sandbox — `.tooling/flutter` dart binary is wrong-arch (Exec format error, consistent with prior sessions) and the GitNexus CLI could not npm-install (registry timeout). Board entry lists the exact five verification commands per the T-068 sandbox-blocked protocol. Impact reasoning (manual): new module + additive MainActivity channel; no existing Dart symbol edited; blast radius limited to MainActivity registration and the build files.
- Decisions: model passed to MediaPipe as a mapped ByteBuffer (files-dir model; `setModelAssetPath` is APK-assets-only); channel returns DoubleArray (StandardMessageCodec-safe) and Dart converts to Float32List for the schema's float32 LE BLOB contract; download is plain single-shot HTTP (6 MB — resumability reserved for the 1.6 GB T-075 download).
- Next: run the five verification commands on the dev machine; then T-050 review; then T-051 (resolver) which consumes `embed()` + `merchants.embedding`.
# 2026-07-11 @codex — T-050 verification and review

- Did: verified the completed on-device embedder implementation, fixed two trailing-comma lint findings in its integration test, confirmed real-device deterministic output, and backfilled ADR 0007's SHA-256 and output dimension.
- Files: integration_test/embedder_determinism_test.dart, docs/decisions/0007-on-device-embedding-model.md, TASKS.md, WORKLOG.md.
...
# 2026-07-12 @codex — T-051/T-052/T-053/T-054/T-062/T-063 tests

- Did: added the missing test coverage for the already-implemented merchant resolver (T-051), local classifier (T-052), classifier trainer (T-053, `ClassifierTrainer` in lib/enrichment/local_classifier.dart), adaptive decision-threshold policy (T-054), model metrics dev screen (T-062), and the correction-time learned-alias write (T-063, `TransactionRepository.correctWithRule`). Fixed one pre-existing test (`sms_ingestion_test.dart`) whose expected confidence-trail `merchant` block predated the resolver wiring — it now asserts the real `unembedded` fallback shape the no-platform-channel test host produces. Moved all six tasks from Ready/Phase-3-backlog to In Review.
- Files: test/enrichment/merchant_resolver_test.dart, test/enrichment/local_classifier_test.dart, test/enrichment/decision_policy_test.dart, test/features/dev/model_metrics_screen_test.dart, test/data/repositories/transaction_repository_test.dart, test/capture/sms_ingestion_test.dart, TASKS.md, WORKLOG.md.
- Evidence: `HOME="$PWD/.tooling/_home" .tooling/flutter/bin/flutter analyze --no-pub` clean; focused run over the five new/changed test files 51/51; full `HOME="$PWD/.tooling/_home" .tooling/flutter/bin/flutter test --no-pub --concurrency=1` 267/267; `git diff --check` clean. GitNexus `detect_changes(scope: all)` reports HIGH — this is the pre-existing (uncommitted, not authored this session) T-051..T-063 production wiring across `SmsIngestor`, `Categorizer`, `DecisionPolicy`, `TransactionRepository`, and `_HomeShellState` touching 8 cross-community processes; this session only added/adjusted tests around that wiring, no further production symbols changed.
