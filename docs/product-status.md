...
| Backup/import | v3 encrypted archive includes all 12 schema tables; 12-character export floor; bounded 32 MiB encrypted file, 16 MiB decoded payload, 50,000-row/table and 200,000-row/archive limits; only non-expired raw SMS is exported/restored; shipped Argon2id profile is required | Whole archive is still held in memory; chunked authenticated streaming, progress, and picker compatibility remain T-127 |
